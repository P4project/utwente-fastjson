...
//        }
    ...
    private long readMillis(char ch, int index, long millis) {
        millis = ch - '0';
        for (; ; ) {
            ch = charAt(index++);
            if (ch >= '0' && ch <= '9') {
    ...
    private java.util.Date scanNegDate(int index, char ch, int startPos, char startChar) {
        long millis = 0;
        boolean negative = false;
        if (ch == '-') {
            ch = charAt(index++);
    ...
    public java.util.Date scanDate(char separator) {
        ...
        if (ch == '"') {
            ...
            dateVal = scanNegDate(index, ch, startPos, startChar);
        ...
        return continueScanDate(ch, dateVal, startPos, startChar);
    }

    private java.util.Date continueScanDate(char ch, java.util.Date dateVal, int startPos, char startChar) {
        if (ch == ',') {
            this.ch = charAt(++bp);
            matchStat = VALUE;
            return dateVal;

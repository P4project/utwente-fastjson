...
public class Jdk8DateCodec extends ContextObjectDeserializer implements ObjectSerializer, ContextObjectSerializer, ObjectDeserializer {
    ...
    private DateTimeFormatter getAsianDateTimeFormat (String text){
        char c4 = text.charAt(4);
        if (c4 == '年') {
            if (text.charAt(text.length() - 1) == '秒') {
                return formatter_dt19_cn_1;
            } else {
                return formatter_dt19_cn;
            }
        } else if (c4 == '년') {
    ...
    private DateTimeFormatter getWesternDateTimeFormat (String text) {
        ...
        if (c2 == '/' && c5 == '/') { // mm/dd/yyyy or mm/dd/yyyy
            int v0 = (text.charAt(0) - '0') * 10 + (text.charAt(1) - '0');
            int v1 = (text.charAt(3) - '0') * 10 + (text.charAt(4)- '0');
            if (v0 > 12) {
                return formatter_dt19_eur;
            } else if (v1 > 12) {
                return formatter_dt19_us;
            } else {
            ...
            return formatter_dt19_de;
        } else if (c2 == '-' && c5 == '-') { // dd-mm-yyyy
            return formatter_dt19_in;
        }
        return null;
    }

    private DateTimeFormatter getDateTimeFormatter(String text) {
        if (text.length() == 19) {
            char c4 = text.charAt(4);
            ...
            if (text.charAt(13) == ':' && text.charAt(16) == ':') {
                if (c4 == '-' && c7 == '-' && text.charAt(10) == 'T') {
                    ...
                    return getWesternDateTimeFormat(text);
                }
            }
        }
    ...
    protected LocalDateTime parseDateTime(String text, DateTimeFormatter formatter) {
        if (formatter == null) {
            formatter = getDateTimeFormatter(text);
        }
